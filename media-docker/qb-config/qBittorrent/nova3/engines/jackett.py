...
CONFIG_DATA: Dict[str, Any] = {
    "api_key": "JACKETT_API_KEY",  # jackett api
...
class jackett:
    name = "Jackett"
    url = (
        CONFIG_DATA["url"] if CONFIG_DATA["url"][-1] != "/" else CONFIG_DATA["url"][:-1]
    )
    ...
    def search_jackett_indexer(
        self, what: str, category: Union[List[str], None], indexer_id: str
    ) -> None:
        ...
        jacket_url = (
            f"{self.url}/api/v2.0/indexers/{indexer_id}/results/torznab/api?{params}"
        )
        response = self.get_response(jacket_url)
        if response is None:
        ...
        for result in channel.findall("item"):
            ...
            res["size"] = (
                -1 if res["size"] is None else (toStr(res["size"].text) + " B")
            )

            res["seeds"] = result.find(self.generate_xpath("seeders"))
            res["seeds"] = (
                -1 if res["seeds"] is None else int(res["seeds"].attrib["value"])
            )

            res["leech"] = result.find(self.generate_xpath("peers"))
            res["leech"] = (
                -1 if res["leech"] is None else int(res["leech"].attrib["value"])
            )
            ...
            if res["desc_link"] is not None:
                ...
                res["desc_link"] = (
                    "" if res["desc_link"] is None else res["desc_link"].text
                )
            ...
            try:
                date = datetime.strptime(
                    getTextProp(result.find("pubDate")), "%a, %d %b %Y %H:%M:%S %z"
                )
